...
async fn main() -> anyhow::Result<()> {
    ...
    let id1 = user1.insert_sqlite(pool.sqlite_pool()).await?;
    println!("插入成功，ID: {}\n", id1);
    ...
    let id2 = user2.insert_sqlite(pool.sqlite_pool()).await?;
    println!("插入成功，ID: {}\n", id2);
    ...
    let id3 = user3.insert_sqlite(pool.sqlite_pool()).await?;
    println!("插入成功，ID: {}\n", id3);

    // ========== 2. FIND_BY_ID (根据 ID 查找) ==========
    println!("=== 2. FIND_BY_ID (根据 ID 查找) ===");
    let found = User::find_by_id_sqlite(pool.sqlite_pool(), id1).await?;
    println!(
        "找到用户: {:?}\n",
    ...
    let users = User::find_by_ids_sqlite(pool.sqlite_pool(), vec![id1, id2, id3]).await?;
    ...
    let builder = QueryBuilder::new("SELECT * FROM user")
        .and_eq("id", id1)
        .order_by("id", false);
    let one_user = User::find_one_sqlite(pool.sqlite_pool(), builder).await?;
    println!(
        "find_one 结果: {:?}\n",
    ...
    let builder = QueryBuilder::new("SELECT * FROM user");
    let total = User::count_sqlite(pool.sqlite_pool(), builder).await?;
    println!("未删除的记录数: {}\n", total);

    // ========== 6. UPDATE (更新 - Patch 语义) ==========
    println!("=== 6. UPDATE (更新 - Patch 语义) ===");
    if let Some(mut user) = User::find_by_id_sqlite(pool.sqlite_pool(), id1).await? {
        user.email = Some(format!("updated_{}@example.com", timestamp));
        user.system_type = Some(2i16);
        user.update_sqlite(pool.sqlite_pool()).await?;
    ...
    if let Some(mut user) = User::find_by_id_sqlite(pool.sqlite_pool(), id1).await? {
        ...
        user.update_with_none_sqlite(pool.sqlite_pool()).await?;
    ...
    let all_users = User::find_all_sqlite(pool.sqlite_pool(), Some(builder)).await?;
    println!("find_all 返回 {} 条记录\n", all_users.len());

    // ========== 9. PAGINATE (分页查询) ==========
    println!("=== 9. PAGINATE (分页查询) ===");
    let builder = QueryBuilder::new("SELECT * FROM user").order_by("id", false);
    let page = User::paginate_sqlite(pool.sqlite_pool(), builder, 1, 10).await?;
    println!(
        "分页结果: 总数={}, 当前页={} 条\n",
        page.total,
        page.items.len()
    ...
    User::soft_delete_by_id_sqlite(pool.sqlite_pool(), id2).await?;
    println!("逻辑删除 ID={} 成功", id2);

    // 验证逻辑删除后 find_by_id 返回 None
    let deleted = User::find_by_id_sqlite(pool.sqlite_pool(), id2).await?;
    if deleted.is_none() {
    ...
    User::hard_delete_by_id_sqlite(pool.sqlite_pool(), id3).await?;
    println!("物理删除 ID={} 成功", id3);

    // 验证物理删除后记录不存在
    let deleted = User::find_by_id_sqlite(pool.sqlite_pool(), id3).await?;
    if deleted.is_none() {
    ...
    let count = User::count_sqlite(pool.sqlite_pool(), builder).await?;
    ...
    println!("=== 13. TRANSACTION - 手动事务（成功提交） ===");
    {
        ...
        let tx_id1 = tx_user1.insert_sqlite(tx.as_sqlite_executor()).await?;
        println!("事务中插入记录，ID: {}", tx_id1);

        // 在事务中更新记录
        if let Some(mut user) = User::find_by_id_sqlite(tx.as_sqlite_executor(), tx_id1).await? {
            user.email = Some(format!("tx_updated_{}@example.com", timestamp));
            user.update_sqlite(tx.as_sqlite_executor()).await?;
            println!("事务中更新记录成功");
        }
        ...
        // 验证事务提交后的数据
        let committed_user = User::find_by_id_sqlite(pool.sqlite_pool(), tx_id1).await?;
    ...
    let rollback_id = {
        ...
        let tx_id2 = tx_user2.insert_sqlite(tx.as_sqlite_executor()).await?;
        println!("事务中插入记录，ID: {}", tx_id2);

        // 在事务中查询记录（应该能查到）
        let tx_user = User::find_by_id_sqlite(tx.as_sqlite_executor(), tx_id2).await?;
        if tx_user.is_some() {
            println!("事务中可以查询到记录");
        }
    ...
    // 验证事务回滚后的数据（应该查询不到）
    let rolled_back_user = User::find_by_id_sqlite(pool.sqlite_pool(), rollback_id).await?;
    ...
    let closure_id = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            let closure_id = closure_user.insert_sqlite(tx.as_sqlite_executor()).await?;
            println!("闭包事务中插入记录，ID: {}", closure_id);

            // 在事务中更新记录
            if let Some(mut user) =
                User::find_by_id_sqlite(tx.as_sqlite_executor(), closure_id).await?
            {
            ...
            let count = User::count_sqlite(tx.as_sqlite_executor(), count_builder).await?;
    ...
    // 验证闭包事务提交后的数据
    let closure_user = User::find_by_id_sqlite(pool.sqlite_pool(), closure_id).await?;
    ...
    let rollback_result: Result<i64, sqlxplus::error::SqlxPlusError> =
        sqlxplus::with_transaction(&pool, |tx| {
            Box::pin(async move {
                ...
                let rollback_id = rollback_user.insert_sqlite(tx.as_sqlite_executor()).await?;
                println!("闭包事务中插入记录，ID: {}", rollback_id);

                // 在事务中查询记录（应该能查到）
                let tx_user = User::find_by_id_sqlite(tx.as_sqlite_executor(), rollback_id).await?;
                if tx_user.is_some() {
    ...
    let (complex_id1, complex_id2) = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            let id1 = user1.insert_sqlite(tx.as_sqlite_executor()).await?;
            ...
            let id2 = user2.insert_sqlite(tx.as_sqlite_executor()).await?;
            println!("插入第二条记录，ID: {}", id2);

            // 更新第一条记录
            if let Some(mut u) = User::find_by_id_sqlite(tx.as_sqlite_executor(), id1).await? {
                u.email = Some(format!("complex_updated1_{}@example.com", timestamp));
                u.update_sqlite(tx.as_sqlite_executor()).await?;
                println!("更新第一条记录成功");
            }

            // 查询多条记录
            let ids = vec![id1, id2];
            let users = User::find_by_ids_sqlite(tx.as_sqlite_executor(), ids).await?;
            println!("查询到 {} 条记录", users.len());

            // 统计记录数
            let builder = QueryBuilder::new("SELECT * FROM user").and_in("id", vec![id1, id2]);
            let count = User::count_sqlite(tx.as_sqlite_executor(), builder).await?;
            println!("统计记录数: {}", count);
    ...
    // 验证复杂事务提交后的数据
    let complex_user1 = User::find_by_id_sqlite(pool.sqlite_pool(), complex_id1).await?;
    let complex_user2 = User::find_by_id_sqlite(pool.sqlite_pool(), complex_id2).await?;
