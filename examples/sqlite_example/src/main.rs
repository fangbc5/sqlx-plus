...
async fn main() -> anyhow::Result<()> {
    ...
    let closure_id = sqlxplus::transaction!(&mut pool, |tx| async move {
        println!("闭包事务开始");
    ...
    let rollback_result: Result<i64, anyhow::Error> =
        sqlxplus::transaction!(&mut pool, |tx| async move {
            println!("闭包事务开始（将回滚）");
    ...
    let (complex_id1, complex_id2) = sqlxplus::transaction!(&mut pool, |tx| async move {
        println!("复杂事务开始");
