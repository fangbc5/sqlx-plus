...
async fn main() -> anyhow::Result<()> {
    ...
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://neondb_owner:npg_UdCbYi0Najg1@ep-dry-band-a1ak7yo1-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require".to_string()
    });
    ...
    let id1 = user1.insert(pool.pg_pool()).await?;
    println!("插入成功，ID: {}\n", id1);
    ...
    let id2 = user2.insert(pool.pg_pool()).await?;
    println!("插入成功，ID: {}\n", id2);
    ...
    let id3 = user3.insert(pool.pg_pool()).await?;
    println!("插入成功，ID: {}\n", id3);

    // ========== 2. FIND_BY_ID (根据 ID 查找) ==========
    println!("=== 2. FIND_BY_ID (根据 ID 查找) ===");
    let found = User::find_by_id(pool.pg_pool(), id1).await?;
    println!(
        "找到用户: {:?}\n",
    ...
    let users = User::find_by_ids(pool.pg_pool(), vec![id1, id2, id3]).await?;
    ...
    let builder = QueryBuilder::new("SELECT * FROM \"user\"")
        .and_eq("id", id1)
        .order_by("id", false);
    let one_user = User::find_one(pool.pg_pool(), builder).await?;
    println!(
        "find_one 结果: {:?}\n",
    ...
    let builder = QueryBuilder::new("SELECT * FROM \"user\"");
    let total = User::count(pool.pg_pool(), builder).await?;
    println!("未删除的记录数: {}\n", total);

    // ========== 6. UPDATE (更新 - Patch 语义) ==========
    println!("=== 6. UPDATE (更新 - Patch 语义) ===");
    if let Some(mut user) = User::find_by_id(pool.pg_pool(), id1).await? {
        user.email = Some(format!("updated_{}@example.com", timestamp));
        user.system_type = Some(2i16);
        user.update(pool.pg_pool()).await?;
    ...
    if let Some(mut user) = User::find_by_id(pool.pg_pool(), id1).await? {
        ...
        user.update_with_none(pool.pg_pool()).await?;
    ...
    let all_users = User::find_all(pool.pg_pool(), Some(builder)).await?;
    println!("find_all 返回 {} 条记录\n", all_users.len());

    // ========== 9. PAGINATE (分页查询) ==========
    println!("=== 9. PAGINATE (分页查询) ===");
    let builder = QueryBuilder::new("SELECT * FROM \"user\"").order_by("id", false);
    let page = User::paginate(pool.pg_pool(), builder, 1, 10).await?;
    println!(
        "分页结果: 总数={}, 当前页={} 条\n",
        page.total,
        page.items.len()
    ...
    User::soft_delete_by_id(pool.pg_pool(), id2).await?;
    println!("逻辑删除 ID={} 成功", id2);

    // 验证逻辑删除后 find_by_id 返回 None
    let deleted = User::find_by_id(pool.pg_pool(), id2).await?;
    if deleted.is_none() {
    ...
    User::hard_delete_by_id(pool.pg_pool(), id3).await?;
    println!("物理删除 ID={} 成功", id3);

    // 验证物理删除后记录不存在
    let deleted = User::find_by_id(pool.pg_pool(), id3).await?;
    if deleted.is_none() {
    ...
    let count = User::count(pool.pg_pool(), builder).await?;
    ...
    println!("=== 13. TRANSACTION - 手动事务（成功提交） ===");
    {
        ...
        let tx_id1 = tx_user1.insert(tx.as_postgres_executor()).await?;
        ...
        if let Some(mut user) = User::find_by_id(tx.as_postgres_executor(), tx_id1).await? {
            ...
            user.update(tx.as_postgres_executor()).await?;
            println!("事务中更新记录成功");
        }
        ...
        // 验证事务提交后的数据
        let committed_user = User::find_by_id(pool.pg_pool(), tx_id1).await?;
    ...
    let rollback_id = {
        ...
        let tx_id2 = tx_user2.insert(tx.as_postgres_executor()).await?;
    ...
    // 验证事务回滚后的数据（应该查询不到）
    let rolled_back_user = User::find_by_id(pool.pg_pool(), rollback_id).await?;
    ...
    let closure_id = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            let closure_id = closure_user.insert(tx.as_postgres_executor()).await?;
    ...
    // 验证闭包事务提交后的数据
    let closure_user = User::find_by_id(pool.pg_pool(), closure_id).await?;
    ...
    let rollback_result: Result<i64, sqlxplus::SqlxPlusError> =
        sqlxplus::with_transaction(&pool, |tx| {
            Box::pin(async move {
                ...
                let rollback_id = rollback_user.insert(tx.as_postgres_executor()).await?;
    ...
    let (complex_id1, complex_id2) = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            let id1 = user1.insert(tx.as_postgres_executor()).await?;
            ...
            let id2 = user2.insert(tx.as_postgres_executor()).await?;
            ...
            if let Some(mut u) = u_opt {
                ...
                u.update(tx.as_postgres_executor()).await?;
                println!("更新第一条记录成功");
            }
    ...
    // 验证复杂事务提交后的数据
    let complex_user1 = User::find_by_id(pool.pg_pool(), complex_id1).await?;
    let complex_user2 = User::find_by_id(pool.pg_pool(), complex_id2).await?;
    ...
    let nested_id = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            let parent_id = parent_user.insert(tx.as_postgres_executor()).await?;
            ...
            let nested_id = sqlxplus::with_postgres_nested_transaction(tx, |nested_tx| {
                Box::pin(async move {
                    ...
                    let nested_id = nested_user.insert(nested_tx.as_postgres_executor()).await?;
                    println!("子事务中插入记录，ID: {}", nested_id);

                    // 在子事务中查询记录
                    let nested_found =
                        User::find_by_id(nested_tx.as_postgres_executor(), nested_id).await?;
            ...
            // 在父事务中验证子事务插入的记录
            let nested_found = User::find_by_id(tx.as_postgres_executor(), nested_id).await?;
            if nested_found.is_some() {
    ...
    // 验证子事务提交后的数据
    let nested_user = User::find_by_id(pool.pg_pool(), nested_id).await?;
    ...
    let parent_user = User::find_by_id(pool.pg_pool(), parent_id_final).await?;
