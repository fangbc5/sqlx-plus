...
pub struct User2 {
    ...
    // ========== 高精度数值类型 ==========
    /// decimal 类型 | 非空 | 默认值: 99.99
    /// 注意：SQLite 不支持 BigDecimal，在 SQLite 中需要使用 skip 属性或条件编译
    #[cfg(not(feature = "sqlite"))]
    #[column(not_null, default = "99.99", comment = "DECIMAL类型 - 非空")]
    pub decimal_field: Option<bigdecimal::BigDecimal>,

    /// numeric 类型 | 可空 | 默认值: 123.456
    /// 注意：SQLite 不支持 BigDecimal，在 SQLite 中需要使用 skip 属性或条件编译
    #[cfg(not(feature = "sqlite"))]
    #[column(default = "123.456", comment = "NUMERIC类型 - 可空")]
    pub numeric_field: Option<bigdecimal::BigDecimal>,
