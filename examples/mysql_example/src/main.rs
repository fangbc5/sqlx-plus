...
async fn main() -> anyhow::Result<()> {
    ...
    // 使用自动类型推断版本（无需显式指定数据库类型）
    let found = User::find_by_id(pool.mysql_pool(), id1).await?;
    println!(
        "找到用户: {:?}\n",
    ...
    let users = User::find_by_ids(pool.mysql_pool(), vec![id1, id2, id3]).await?;
    ...
    let builder = QueryBuilder::new("SELECT * FROM user")
        .and_eq("id", id1)
        .order_by("id", false);
    let one_user = User::find_one(pool.mysql_pool(), builder).await?;
    println!(
        "find_one 结果: {:?}\n",
    ...
    let builder = QueryBuilder::new("SELECT * FROM user");
    let total = User::count(pool.mysql_pool(), builder).await?;
    println!("未删除的记录数: {}\n", total);
    ...
    let all_users = User::find_all(pool.mysql_pool(), Some(builder)).await?;
    ...
    let deleted = User::find_by_id(pool.mysql_pool(), id2).await?;
    if deleted.is_none() {
    ...
    // 验证物理删除后记录不存在
    let deleted = User::find_by_id(pool.mysql_pool(), id3).await?;
    if deleted.is_none() {
    ...
    println!("=== 13. TRANSACTION - 手动事务（成功提交） ===");
    {
        ...
        // 验证事务提交后的数据
        let committed_user = User::find_by_id(pool.mysql_pool(), tx_id1).await?;
    ...
    let rollback_id = {
        ...
        // 在事务中查询记录（应该能查到）
        let tx_user = User::find_by_id(tx.as_mysql_executor(), tx_id2).await?;
        if tx_user.is_some() {
            println!("事务中可以查询到记录");
        }
    ...
    // 验证事务回滚后的数据（应该查询不到）
    let rolled_back_user = User::find_by_id(pool.mysql_pool(), rollback_id).await?;
    ...
    let closure_id = sqlxplus::with_transaction(&pool, |tx| {
        Box::pin(async move {
            ...
            // 在事务中更新记录（使用自动类型推断）
            // 注意：需要使用 as_mysql_executor() 而不是 as_mysql_transaction()
    ...
    // 验证闭包事务提交后的数据
    let closure_user = User::find_by_id(pool.mysql_pool(), closure_id).await?;
    ...
    // 验证复杂事务提交后的数据
    let complex_user1 = User::find_by_id(pool.mysql_pool(), complex_id1).await?;
    let complex_user2 = User::find_by_id(pool.mysql_pool(), complex_id2).await?;
        ...
        Box::pin(async move {
            ...
            let nested_id = sqlxplus::with_mysql_nested_transaction(tx, |nested_tx| {
                Box::pin(async move {
                    ...
                    // 在子事务中查询记录
                    let nested_found =
                        User::find_by_id(nested_tx.as_mysql_executor(), nested_id).await?;
            ...
            // 在父事务中验证子事务插入的记录
            let nested_found = User::find_by_id(tx.as_mysql_executor(), nested_id).await?;
            if nested_found.is_some() {
    ...
    // 验证子事务提交后的数据
    let nested_user = User::find_by_id(pool.mysql_pool(), nested_id).await?;
    ...
    let parent_user = User::find_by_id(pool.mysql_pool(), parent_id_final).await?;
    ...
    let builder_id = builder_user.insert(pool.mysql_pool()).await?;
    ...
    // 验证更新结果
    let updated_user = User::find_by_id(pool.mysql_pool(), builder_id).await?;
    ...
    // 验证插入结果
    let inserted_user = User::find_by_id(pool.mysql_pool(), insert_id).await?;
    ...
    let delete_id1 = delete_user1.insert(pool.mysql_pool()).await?;
    ...
    let delete_id2 = delete_user2.insert(pool.mysql_pool()).await?;
    ...
    // 验证删除结果
    let deleted_user = User::find_by_id(pool.mysql_pool(), delete_id1).await?;
