...
1. **条件分组（括号）支持**
   ...
   - 当前 OR/AND 条件都是线性的
   - 需要：支持条件分组，如 `(A AND B) OR (C AND D)`
   - 示例：`builder.and_group(|b| b.and_eq("a", 1).and_eq("b", 2)).or_group(...)`
...
- **条件组合**: 85% ✅（支持 AND/OR，缺少条件分组括号）
- **高级查询**: 30% ⚠️（缺少 JOIN、子查询）
- **聚合查询**: 10% ❌（基本不支持）
- **复杂查询**: 20% ❌（缺少 UNION、EXISTS 等）

**总体评估**: 当前 QueryBuilder 能够覆盖 **75-85%** 的常见查询场景，非常适合大多数 CRUD 操作和中等复杂度的查询。已实现的高优先级功能大大提升了实用性。
